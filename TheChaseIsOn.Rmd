...
ggplot(plot_data, aes(fill=OfferTaken, y=count, x=cuts)) + 
  ...
  theme_campbead()
```
Likelihood of winning HTH based on chosen value
```{r}
library(scales)
source("custom_gg.R")
...
ggplot(plot_data, aes(color=OfferTaken, y=win_pct, x=ChosenOffer, size = total_count)) +
  geom_point() +
  scale_color_manual(values = my_colors) +
  xlim(-5000,15000) +
  #ylim(0,1)+
  xlab("Chosen Offer (£)") +
  scale_y_continuous(name = "Win Percentage", labels = percent,  expand = expansion(mult = c(0.25,0.05))) +
  theme_campbead()
